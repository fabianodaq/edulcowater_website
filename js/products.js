...
const products = {
    ...
    'Modular Plug': {
        ...
        specs: {
            'Power supply': ['Power supply ⚡', '230V AC'],
            'Inputs': ['Inputs 🔌', 'Drive 5V DC USB-A'],
    ...
    'Smart Plug EC/Temp': {
        ...
        specs: {
            ...
            'TDS': ['TDS 💎', '6 – 12,800 ppm'],
            'Salinity': ['Salinity 🧂', '6 – 11,000 ppm'],
            'Temperature': ['Temperature 🌡️', 'analog 0-150°C'],
            'Control type': ['Control Type ⚙️', 'ON-OFF with Hysteresis, logic Positive/negative, Timer associated'],
            'Mounting': ['Mounting 🛠️', 'Power Plug'],
            'Dimensions': ['Dimensions 📐', '66 × 93 × 93 mm'],
            'Connectivity': ['Connectivity 📡', 'WiFi in 🔗Access Point (No Router) and in 🌐 Station (with router)'],
    ...
    'Smart Sens pH/ORP': {
        ...
        specs: {
            'Power supply': ['Power supply ⚡', '5V DC'],
            'pH': ['pH 💧', '0.01 – 14.00'],
            'ORP': ['ORP ⚗️', '-1800 – +1800 mV'],
            'Control type': ['Control Type ⚙️', 'ON-OFF with Hysteresis, logic Positive/negative, Timer associated'],
            'Mounting': ['Mounting 🛠️', 'Power Plug'],
...
const loadProductCards = () => {
    document.querySelectorAll('.product-card').forEach((productCard) => {
        ...
        productCard.querySelector('.product-name').textContent = product[`title_${language.toLowerCase()}`] || productName;
...
const openProductDetails = (productName) => {
    ...
    const language = localStorage.getItem('edulco_language') || 'EN';
    productModal.querySelector('#product-modal-title').textContent = product[`title_${language.toLowerCase()}`] || productName;
